...
def main():
    ...
    if config.getboolean("bilby_pipe_settings", "injection"):
        inject_file = gravelamps.lensing.utils.gen_inject_file(config, injection_parameters)
    else:
        inject_file = None
    ...
    if config.getboolean("data_settings", "create_unlensed_prep_run"):
        ...
        unlensed_ini = gravelamps.lensing.utils.gen_bilby_pipe_ini(
    ...
    lensed_ini = gravelamps.lensing.utils.gen_bilby_pipe_ini(
            config=config, inject_file=inject_file,
            waveform_arguments=waveform_arguments,
            mode="lensed")
    ...
    dag_file = gravelamps.lensing.utils.gen_overarch_dag(config)
